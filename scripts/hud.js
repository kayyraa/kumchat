...
const FetchDMs = async () => {
    ...
    Documents.forEach(async (Document) => {
        ...
        Node.addEventListener("click", function() {
            ...
            function UpdateMessages() {
                ...
                Document.Messages.forEach(async (Message, Index, Messages) => {
                    ...
                    MessageNode.innerHTML = `
                        ...
                        ${Message.Reply ? `
                            <div original="${String(JSON.parse(Message.Reply).Content).replace("|", "").replaceAll("|", "")}" class="Reply">
                        ...
                        ` : ""}
                        ${Message.Forwarded ? `<span class="Forwarded">Forwarded from <i>"${Message.Forwarded[1]}"</i></span>` : ""}
                    `;

                    if (Math.abs(LastTimestamp - Message.Timestamp) >= (60 * 60 * 24)) {
                        const DayDivider = document.createElement("div");
                        DayDivider.setAttribute("class", "DayDivider");
                        DayDivider.style.order = Message.Timestamp;
                        DayDivider.innerHTML = new Format(Message.Timestamp).ConvertEpochToReadable("hh:mm dd mn")
                        Chat.querySelector(".Messages").appendChild(DayDivider);
                    }
                    ...
                    MessageNode.ClickAndHold((Event) => {
                        ...
                        if (IsAuthor) {
                            ...
                            <div class="ReportButton">
                                <span>Report</span>
                                <span style="font-family: 'SFSymbolsFallback'">􀋉</span>
                            </div>`;

                            requestAnimationFrame(() => {
                                Dropdown.querySelector(".ReportButton").addEventListener("click", () => {
                                    Functions.Report(`${MessageNode.querySelector("span:nth-child(1)").textContent}: ${TruncateString(MessageNode.querySelector("span:nth-child(2)").textContent, 24, "...")}`, "../images/Flag.png")
                                });
                            });
                        }
                        ...
                        <div class="ReplyButton">
                            <span>Reply</span>
                            <span style="font-family: 'SFSymbolsFallback'">􀉌</span>
                        </div>
                        <div class="ForwardButton">
                            <span>Forward</span>
                            <span style="font-family: 'SFSymbolsFallback'">􀉔</span>
                        </div>
                        <div class="ReactButton">
                            ...
                            <div>
                                ...
                                <img button name="Fire" extension=".svg" src="../images/reactions/Fire.svg">
                        ...
                        Dropdown.querySelector(".ForwardButton").addEventListener("click", async () => {
                            Dropdown.innerHTML = "";

                            let ForwardContent = Array.from(MessageNode.querySelector("span:nth-child(2)").children)
                            ...
                            Documents.forEach(async Document => {
                                ...
                                Node.innerHTML = `
                                    <span>${Name}</span>
                                    <img style="border-radius: 100%;" src="${Icon}">
                                `;
                                ...
                                Node.addEventListener("click", async () => {
                                    ...
                                    const NewMessage = {
                                        ...
                                        Reactions: [],
                                        Forwarded: [true, ActiveDm.querySelector("div").querySelector("span:nth-child(1)").textContent],
                                    };

                                    await new FireStorage("DMs").UpdateDocument(Document.id, {
                ...
                setTimeout(() => Chat.querySelector(".Messages").scrollTop = Chat.querySelector(".Messages").scrollHeight, 500);
            }
            ...
            let Label = null;
            let Dumped = false;
            Chat.querySelector(".Input .MicButton").addEventListener("click", async () => {
                ...
                if (!Recording) {
                    ...
                    const Support = await NewRecord.Start();
                    if (Support === false) {
                        Recording = false;
                        NewRecord = null;
                        Label.style.opacity = "0";
