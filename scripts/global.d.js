...
/**
 * # Glyph
 * - A class to manage layers of HTML elements or images.
 * - Can append, update, and modify the visibility of layers.
 * - Useful for dynamic creation and manipulation of layered elements or images.
 */
globalThis.Glyph = class {
    /**
     * Creates a Glyph instance with a set of layers.
     * @param {Array<HTMLElement | string>} Layers - An array of layers, where each layer can be an HTML element or a string URL for an image.
     */
    constructor(Layers = []) {
        this.Layers = Layers;
        this.Glyph = null;
    ...
    /**
     * Appends the Glyph to a parent element and adds the layers.
     * @param {HTMLElement} Parent - The parent HTML element to append the Glyph to.
     * @returns {HTMLDivElement} - The created HTML div element containing the layers.
     */
    Append(Parent) {
        if (!this.Layers) return;
        this.Glyph = document.createElement("div");
        this.Glyph.setAttribute("class", "Glyph");
        this.Layers.forEach((Layer, index) => {
            if (typeof Layer === "string") {
                const Image = document.createElement("img");
                Image.src = Layer;
                this.Glyph.appendChild(Image);
                // Replace the string with the image element in the Layers array
                this.Layers[index] = Image;
            } else if (Layer instanceof HTMLElement) {
                this.Glyph.appendChild(Layer);
            }
        });
        Parent.appendChild(this.Glyph);
        return this.Glyph;
    }

    /**
     * Adds a new layer to the Glyph.
     * @param {HTMLElement | string} Layer - A new layer to add, either an HTML element or an image URL.
     * @returns {HTMLDivElement} - The updated HTML div element containing the added layer.
     */
    Add(Layer) {
        if (!Layer || !this.Glyph || !this.Layers) return;
    ...
    /**
     * Applies style to a specific or every layer from the Glyph.
     * @param {number} SpecificLayerIndex - The index of the layer to apply the style to. If not provided, styles will be applied to all layers.
     * @param {(string | object)} Style - The style to apply. Can be a string (CSS style string) or an object where keys are CSS properties and values are the corresponding CSS values.
     */
    ApplyStyle(SpecificLayerIndex = 0, Style = {}) {
        if (!this.Glyph || !this.Layers) return;

        if (SpecificLayerIndex) {
            const Layer = this.Layers[SpecificLayerIndex];
            if (!(Layer instanceof HTMLElement)) return;

            if (typeof Style === "object") {
                Object.keys(Style).forEach(Key => Layer.style[Key] = Style[Key]);
    ...
    /**
     * Removes a layer from the Glyph.
     * @param {HTMLElement} Layer - The layer to remove.
     */
    Remove(Layer) {
        if (!Layer || !this.Glyph || !this.Layers) return;
    ...
    /**
     * Deletes the entire Glyph from the DOM.
     */
    Delete() {
...
globalThis.Beacon = class {
    constructor(Namespace, Payload) {
    ...
    Send() {
        ...
        navigator.sendBeacon(this.Namespace, this.Payload);
    }
}
